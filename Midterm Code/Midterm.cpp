...
//Descriptions to include for Service State
...
int wmain(int argc, wchar_t* argv[]) {
    ...
    if (cmd == L"query") {
        ...
        if (argc >= 3) {
            ...
            wprintf(L"\nSERVICE_NAME: %ls\n", argv[2]);
            wprintf(L"   TYPE: %x\n", ssp.dwServiceType);
            wprintf(L"   STATE: %lu  %ls\n", ssp.dwCurrentState, StateToString(ssp.dwCurrentState));
    ...
    // Config Command – modify service configuration (type, start, binary path, etc.)
        ...
        SC_HANDLE sch = OpenSCManager(NULL, NULL, SC_MANAGER_CONNECT);
        //Open Service with config permissions
        SC_HANDLE sh = OpenService(sch, serviceName, SERVICE_ALL_ACCESS);
